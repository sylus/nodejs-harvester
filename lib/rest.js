...
exports.rest_get = function(type) {
    ...
    app.get('/' + type + '/:id', function(req, res) {
        database.connection.query('SELECT * FROM ' + type +
            ' INNER JOIN lookup ON ' + type + '.id = lookup.id WHERE ' + type +
            '.id = "' + req.params.id + '"', function(err, result) {
            if (err) {
                console.log(err);
            } else {
                if (params.config.cms === 'aem') {
                    aem.aemify(result);
                }
                res.json(result);
            }
    ...
    app.get('/join/' + type, function(req, res) {
        database.connection.query('SELECT t.id, t.language, l.id AS uuid, l.source, t.website, l.href FROM ' +
            type + ' t INNER JOIN component_links l ON t.website=l.link', function(err, result) {
            if (err) {
                console.log(err);
            } else {
                res.json(result);
    ...
    app.get('/join/' + type + '/:id', function(req, res) {
        database.connection.query('SELECT t.id, t.language, l.id AS uuid, l.source, t.website, l.href FROM ' +
            type + ' t INNER JOIN component_links l ON t.website=l.link WHERE uuid = "' +
    ...
    app.get('/join/language/' + type, function(req, res) {
        database.connection.query('SELECT t.id, t.language, l.id AS uuid, l.source, t.website, l.href FROM ' +
            type + ' t INNER JOIN component_links l ON t.website=l.link WHERE l.href = "' +
...
// Instantiates table from anyDB to REST Route.
exports.rest_get_table = function(type) {
    app.get('/' + type, function(req, res) {
        database.connection.query('SELECT DISTINCT id FROM ' + type +
        ' ORDER BY id ASC', function(err, result) {
            if (err) {
...
var menus = {
    ...
    'resources': [{
        ...
        'path': '/',
        'submenu': [{
            'https://cheerio.js.org': 'CheerioJS',
...
if (params.config.cms === 'aem') {
    menus.adobe = [{
        ...
        'path': '/',
        'submenu': [{
            'https://docs.adobe.com/docs/en/aem/6-2/develop/dev-tools/site-importer.html': 'Site Importer'
        }]
    }];
}
var breadcrumbs = {
