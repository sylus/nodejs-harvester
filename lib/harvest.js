...
function harvest_table(type) {
    ...
    // RESTFUL JSON for media.
    rest.rest_get_table(type);

    // Export DB as JSON file for this type.
    if (process.argv[2] === 'export') {
        exportable.export_table(type);
    }
}
...
function harvest() {
    ...
    harvest_media('component_media');
    harvest_links('component_links');
    harvest_table('component_table');
