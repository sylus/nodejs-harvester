    ...
    request(request_options, function(err, response, html) {
        ...
        function parseHTML(html) {
            ...
            if (rendered.length >= 1) {
                ...
                // Hack to remove images from inside a href
                ...
                //asyncReplace(rendered.toString(), /<a [\S\s]*?>/ig, parseLinksAsync, function(err, result) {
                //  if (err) {
        ...
        function parseIMG(match) {
            ...
            database.connection.query('INSERT INTO media VALUES (' + rowMedia + ')', insertMedia, function(err) {
                if (err) {
                    debug.logger.log('error', 'Insertion media failed due to: ' + err.code);
                } else {
                    image.save();
                }
            });
        ...
        function parseFILE(match) {
            ...
            database.connection.query('INSERT INTO media VALUES (' + rowFile + ')', insertFile, function(err) {
                if (err) {
                    debug.logger.log('error', 'Insertion media failed due to: ' + err.code);
                } else {
                    file.save();
        ...
        function parseLinks(match) {
            ...
            if ((link.link !== null) && (link.host !== null) && link.link.indexOf('NODEJSHARVEST') === -1) {
                ...
                database.connection.query('INSERT INTO links VALUES (' + rowLink + ')', insertLink, errorHandler);
                    ...
                    database.connection.query('SELECT * FROM lookup WHERE website LIKE "%' +
                        ...
                        if (err) {
                            ...
                            if (result.rowCount > 0) {
                                database.connection.query('INSERT INTO links VALUES (' +
