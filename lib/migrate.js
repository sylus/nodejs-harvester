...
exports.migrate = function(row, type) {
    ...
    request(request_options, function(err, response, html) {
        ...
        function parseIMG(match) {
            ...
            var insertMedia = [];
            var rowMedia = ['$1', '$2', '$3', '$4', '$5', '$6'];

            insertMedia.push(image.name);
            insertMedia.push(url);
            insertMedia.push(image.saveTo + imageName + image.extension);
            insertMedia.push(image.name + image.extension);
            insertMedia.push(image.metadata);
        ...
        function parseTABLE(match) {
            ...
            if (params.config.token === 'tag') {
                ...
                return '[NODEJSHARVEST_TABLE:' + table.name + ']';
            }
        }
        ...
        function parseFILE(match) {
            ...
            var insertFile = [];
            var rowFile = ['$1', '$2', '$3', '$4', '$5', '$6'];

            insertFile.push(file.name);
            insertFile.push(url);
            insertFile.push(file.saveTo + fileName + file.extension);
            insertFile.push(file.name + file.extension);
            insertFile.push(file.metadata);
            ...
            if (params.config.token === 'tag') {
                ...
                return '[NODEJSHARVEST_FILE:' + file.name + ']';
            }
        }
        ...
        function parseLINKS(match) {
            ...
            if ((link.link !== null) && (link.host !== null) && link.link.indexOf('NODEJSHARVEST') === -1) {
                ...
                if (params.config.token === 'tag') {
                    ...
                    return '[NODEJSHARVEST_LINK:' + linkUUID + ']';
                }
            } else {
                return match;
            }
        ...
        // Parse HTML for Lists.
        function parseLISTS(match) {
